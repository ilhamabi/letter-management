    ...
    if ($isEmpty) {
        ...
        $submissions = $submissions ?? [
            [
                ...
                'timeline' => [
                    ...
                    [
                        ...
                        'time' => 'SEDANG DIPROSES oleh Heri Setyawan, M.Kom.',
                        'status' => 'active',
                    ],
            ...
            [
                ...
                'lecturer' => 'Dr. Heri Setyawan, M.Kom. (Dosen Wali)',
                'time' => '08:30 WIB',
                'attachments' => [['name' => 'Ijazah_Alex.pdf', 'size' => '2.1 MB']],
                'timeline' => [
            ...
            [
                ...
                'time' => '14:20 WIB',
                'attachments' => [['name' => 'Bebas_Pinjam_Perpus.pdf', 'size' => '510 KB']],
                'timeline' => [
...
<x-app-layout>
    ...
    <div class="bg-surface-gray text-on-surface min-h-screen flex">
        <!-- SideNavBar -->
        <nav
            ...
            <div class="px-container-padding mb-12 flex items-center gap-4">
                ...
                <div>
                    <h1
                        class="font-display-lg text-title-lg text-primary leading-[1.2] font-bold uppercase tracking-tight">
            ...
            <div class="border-t border-outline-variant bg-surface-container-low/30">
                <div class="flex items-center gap-4 px-6 py-4">
                    ...
                    <div class="overflow-hidden">
                        <p class="font-label-md text-body-md text-on-surface truncate font-semibold">{{ $studentName }}
                        </p>
        ...
        <div class="flex-grow md:ml-sidebar-width flex flex-col min-h-screen md:ml-[280px] pt-0">
            <!-- Top AppBar -->
            <header
                class="h-16 bg-pure-white border-b border-outline-variant flex justify-between items-center px-container-padding sticky top-0 z-10 w-full">
                <h2 class="font-headline-sm text-headline-sm text-primary hidden md:block w-auto whitespace-nowrap">
                    Layanan Dokumen</h2>
                    ...
                    <div class="flex items-center gap-4 text-on-surface-variant">
                        @if (Route::has('logout'))
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit"
                                    class="flex items-center gap-2 px-4 py-2 border border-outline-variant rounded-lg text-on-surface-variant font-label-md hover:bg-surface-container transition-colors">
            ...
            <main class="flex-grow p-container-padding flex flex-col gap-8 max-w-6xl">
                <!-- Welcome Section -->
                <section
                    class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-pure-white p-8 rounded-xl border border-outline-variant">
                    <div>
                        <h2 class="font-headline-md text-headline-md text-on-surface mb-2">Selamat datang kembali,
                            {{ $studentName }}</h2>
                        <p class="font-body-md text-body-md text-on-surface-variant">Berikut ikhtisar permintaan dokumen
                            akademik Anda.</p>
                    </div>
                </section>
                ...
                <section class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <!-- Pending -->
                    <div
                        class="bg-pure-white p-6 rounded-xl border border-outline-variant flex items-center gap-5 shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <div
                        ...
                        <div>
                            <p class="font-label-md text-label-md text-on-surface-variant mb-1">Ditolak / Perlu Tindakan
                            </p>
                ...
                <section
                    class="bg-pure-white rounded-xl border border-outline-variant shadow-sm overflow-hidden flex flex-col">
                        ...
                        <div class="overflow-x-auto w-full">
                            <table class="w-full text-left border-collapse">
                                <thead class="bg-surface-container-low">
                                    <tr>
                                        <th
                                            class="font-label-sm text-label-sm uppercase tracking-wider text-on-surface-variant py-4 px-6 font-medium">
                                            Jenis Surat</th>
                                        <th
                                            class="font-label-sm text-label-sm uppercase tracking-wider text-on-surface-variant py-4 px-6 font-medium">
                                            Tanggal Pengajuan</th>
                                        <th
                                            class="font-label-sm text-label-sm uppercase tracking-wider text-on-surface-variant py-4 px-6 font-medium text-center">
                                            Status</th>
                                    </tr>
                                </thead>
                                <tbody
                                    ...
                                    @foreach ($submissions as $sub)
                                        <tr class="hover:bg-surface-container-low transition-colors duration-200 cursor-pointer"
                                            onclick="openStatusModal({{ json_encode($sub) }})">
                                            <td class="py-5 px-6 font-semibold text-deep-black">{{ $sub['type'] }}
                                            </td>
                    ...
                    @else
                        <div class="flex flex-col items-center justify-center py-12 px-6 text-center">
                            <div
                                class="w-16 h-16 bg-surface-container rounded-full flex items-center justify-center text-on-surface-variant mb-4">
                                <span class="material-symbols-outlined text-4xl">description</span>
                            </div>
                            <h4 class="font-headline-sm text-headline-sm text-deep-black mb-2">Tidak ada pengajuan yang
                                sedang berlangsung</h4>
                            <p class="font-body-md text-body-md text-on-surface-variant mb-6">Semua permintaan dokumen
                                Anda telah selesai diproses atau belum ada pengajuan baru.</p>
                            <a class="text-primary font-label-lg text-label-lg hover:underline flex items-center gap-2"
                                href="#">
            ...
            <div
                ...
                <div class="p-6 space-y-6 overflow-y-auto max-h-[60vh]">
                    ...
                    <div class="grid grid-cols-2 gap-y-4 gap-x-8">
                        <div class="space-y-1">
                            <p class="text-[10px] uppercase tracking-widest text-on-surface-variant font-semibold">
                                Keperluan</p>
                            <p class="font-label-lg text-label-lg text-deep-black font-bold" id="modal-purpose">Syarat
                                Beasiswa</p>
                        </div>
                        <div class="space-y-1">
                            <p class="text-[10px] uppercase tracking-widest text-on-surface-variant font-semibold">
                                Dosen Dituju</p>
                            <p class="font-label-lg text-label-lg text-deep-black font-bold" id="modal-lecturer">Dr.
                                Heri Setyawan, M.Kom. (Dosen Wali)</p>
                        </div>
                        <div class="space-y-1">
                            <p class="text-[10px] uppercase tracking-widest text-on-surface-variant font-semibold">NIM
                            </p>
                            <p class="font-label-lg text-label-lg text-deep-black font-bold" id="modal-nim">
                                {{ $nim }}</p>
                        </div>
                        <div class="space-y-1">
                            <p class="text-[10px] uppercase tracking-widest text-on-surface-variant font-semibold">
                                Program Studi</p>
                            <p class="font-label-lg text-label-lg text-deep-black font-bold" id="modal-prodi">
                                {{ $prodi }}</p>
                        </div>
                    ...
                    <div class="bg-primary-fixed/30 p-4 rounded-lg border border-primary/10 flex gap-3">
                        ...
                        <p class="text-body-sm text-on-surface-variant">Proses verifikasi biasanya memakan waktu 1-2
                            hari kerja. Jika belum ada pembaruan, Anda dapat mengirim pengingat.</p>
                    </div>
                </div>

                <div class="p-6 bg-surface-gray border-t border-outline-variant flex justify-end gap-3">
                    <button
                        class="px-6 py-2 border border-error text-error font-label-md rounded-lg hover:bg-error-container transition-colors"
                        onclick="document.getElementById('cancel-confirm-modal').classList.remove('hidden')">Batalkan
                        Pengajuan</button>
                    <button
                        class="px-6 py-2 bg-primary text-white font-label-md rounded-lg hover:shadow-md transition-shadow"
                        onclick="document.getElementById('status-modal').classList.add('hidden')">Tutup</button>
                </div>
            </div>
        </div>
        ...
        <div class="fixed inset-0 z-[60] flex items-center justify-center hidden" id="cancel-confirm-modal">
            <div class="absolute inset-0 bg-black/50 backdrop-blur-sm"
                onclick="document.getElementById('cancel-confirm-modal').classList.add('hidden')"></div>
            <div
                class="relative bg-pure-white w-full max-w-md mx-4 rounded-xl shadow-xl overflow-hidden flex flex-col animate-in fade-in zoom-in duration-200">
                <div class="p-6 border-b border-outline-variant">
                    <h3 class="font-headline-sm text-headline-sm text-deep-black">Batalkan Pengajuan?</h3>
                </div>
                <div class="p-6">
                    <p class="font-body-md text-body-md text-on-surface-variant">Apakah Anda yakin ingin membatalkan
                        pengajuan surat ini? Tindakan ini tidak dapat dibatalkan.</p>
                </div>
                <div class="p-6 bg-surface-gray border-t border-outline-variant flex justify-end gap-3">
                    <button
                        class="px-6 py-2 border border-error text-error font-label-md rounded-lg hover:bg-error-container transition-colors"
                        onclick="alert('Pengajuan berhasil dibatalkan'); document.getElementById('cancel-confirm-modal').classList.add('hidden'); document.getElementById('status-modal').classList.add('hidden');">Ya,
                        Batalkan</button>
                    <button
                        class="px-6 py-2 bg-primary text-white font-label-md rounded-lg hover:shadow-md transition-shadow"
                        onclick="document.getElementById('cancel-confirm-modal').classList.add('hidden')">Kembali</button>
                </div>
            </div>
        </div>
...
<script>
    function openStatusModal(data) {
        ...
        if (data.attachments && data.attachments.length > 0) {
            ...
            attachmentsContainer.innerHTML =
                '<p class="text-body-sm text-on-surface-variant italic">Tidak ada lampiran</p>';
        }
