...
<body class="bg-surface-gray text-on-surface min-h-screen flex">
    ...
    <div class="flex-grow md:ml-sidebar-width flex flex-col min-h-screen md:ml-[280px] pt-0">
        <!-- TopAppBar -->
        <header
            class="bg-pure-white border-b border-outline-variant flex justify-between items-center px-container-padding sticky top-0 z-10 w-full h-16">
            <h2 class="font-headline-sm text-headline-sm text-primary hidden md:block w-64 whitespace-nowrap">Layanan
                Dokumen</h2>
        ...
        <main class="flex-grow p-container-padding flex flex-col gap-8 max-w-6xl">
            ...
            <section class="flex flex-col gap-2">
                ...
                <p class="font-body-md text-body-md text-on-surface-variant">Kelola dan pantau status permohonan dokumen
                    akademik Anda di sini.</p>
            </section>
                    ...
                    <div class="flex items-center gap-2">
                        <div class="relative">
                            <input id="filter-start-date"
                                ...
                                onblur="(this.type='text')" onfocus="(this.type='date')" placeholder="Mulai Tanggal"
                                type="text">
                            <span
            ...
            <section
                        ...
                        <tbody class="divide-y divide-outline-variant font-body-sm text-on-surface"
                            ...
                            @foreach ($submissions as $index => $item)
                                <tr class="submission-row hover:bg-surface-container-low transition-colors duration-200 cursor-pointer"
                                    ...
                                    data-date="{{ $item['date'] }}" onclick="openDetailModal({{ $item['id'] }})">
                                    ...
                                    <td class="px-6 py-5 text-center">
                                        @if ($item['status'] === 'Disetujui')
                                            <span
                                                class="px-3 py-1 bg-green-100 text-green-700 border border-green-200 text-[11px] rounded-full font-semibold uppercase tracking-wider">Disetujui</span>
                                        @elseif ($item['status'] === 'Ditolak')
                                            <span
                                                class="px-3 py-1 bg-error-container text-error border border-error/20 text-[11px] rounded-full font-semibold uppercase tracking-wider">Ditolak</span>
                                        @else
                                            <span
                                                class="px-3 py-1 bg-secondary-container text-secondary border border-yellow-200 text-[11px] rounded-full font-semibold uppercase tracking-wider">{{ $item['status'] }}</span>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
                ...
                <!-- Pagination -->
                <div
                    class="px-6 py-4 bg-surface-container-low flex items-center justify-between border-t border-outline-variant">
                    <p class="text-body-sm text-on-surface-variant font-medium">Menampilkan 1-5 dari
                        {{ count($submissions) }} pengajuan</p>
                    <div class="flex gap-2">
                        <button
                            class="w-9 h-9 flex items-center justify-center rounded-lg border border-outline-variant bg-pure-white text-on-surface-variant hover:border-primary hover:text-primary transition-all">
                            <span class="material-symbols-outlined text-[20px]">chevron_left</span>
                        </button>
                        <button
                            class="w-9 h-9 flex items-center justify-center rounded-lg bg-primary text-on-primary font-bold">1</button>
                        <button
                            class="w-9 h-9 flex items-center justify-center rounded-lg border border-outline-variant bg-pure-white text-on-surface-variant hover:border-primary hover:text-primary transition-all">2</button>
                        <button
                            class="w-9 h-9 flex items-center justify-center rounded-lg border border-outline-variant bg-pure-white text-on-surface-variant hover:border-primary hover:text-primary transition-all">3</button>
                        <button
                            class="w-9 h-9 flex items-center justify-center rounded-lg border border-outline-variant bg-pure-white text-on-surface-variant hover:border-primary hover:text-primary transition-all">
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center hidden" id="status-detail-modal">
        ...
        <div
            class="relative bg-pure-white w-full max-w-lg mx-4 rounded-xl shadow-xl overflow-hidden flex flex-col animate-scale-up">
            <!-- Modal Header -->
            ...
            <div class="p-6 space-y-6 overflow-y-auto max-h-[70vh]">
                ...
                <div id="modal-timeline"
                    class="relative pl-8 space-y-8 before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-0.5 before:bg-outline-variant">
                ...
                <div class="grid grid-cols-2 gap-y-4 gap-x-8">
                    ...
                    <div class="space-y-1">
                        <p class="text-[10px] uppercase tracking-widest text-on-surface-variant font-semibold">Dosen
                            Dituju</p>
                    ...
                    <div class="space-y-1">
                        ...
                        <p id="modal-nim" class="font-label-lg text-label-lg text-deep-black font-bold">
                            {{ $nim }}</p>
                    </div>
                    <div class="space-y-1">
                        <p class="text-[10px] uppercase tracking-widest text-on-surface-variant font-semibold">Program
                            Studi</p>
                        <p id="modal-prodi" class="font-label-lg text-label-lg text-deep-black font-bold">
                            {{ $prodi }}</p>
                    </div>
                </div>
                ...
                <div class="bg-primary-fixed/30 p-4 rounded-lg border border-primary/10 flex gap-3">
                    ...
                    <p class="text-body-sm text-on-surface-variant">Proses verifikasi biasanya memakan waktu 1-2 hari
                        kerja. Jika belum ada pembaruan, Anda dapat menghubungi bagian Akademik.</p>
                </div>
            </div>
            ...
            <div class="p-6 bg-surface-gray border-t border-outline-variant flex justify-end gap-3">
                <button
                    class="px-6 py-2 border border-primary text-primary font-label-md rounded-lg hover:bg-primary-fixed/20 transition-colors flex items-center gap-2"
                    id="download-btn">
