...
<section 
    ...
    <div class="flex justify-between items-center border-b border-gray-100 pb-4 mb-6 mt-2">
        ...
        <button class="bg-amikom-purple text-white px-4 py-2 rounded-lg font-bold transition-all hover:opacity-90 active:scale-95 shadow-sm text-sm font-label-md" onclick="document.getElementById('{{ $modalId }}').classList.remove('hidden')" type="button">Ubah Alur</button>
    </div>
        ...
        <template x-for="(step, index) in currentSteps" :key="index">
            <div class="flex items-center gap-4 shrink-0">
                <div class="flex items-center gap-2 bg-gray-50 p-2.5 rounded-lg border border-gray-200 group shrink-0 min-w-[200px] hover:border-amikom-purple transition-all">
                    <div class="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center text-white text-xs font-bold shadow-sm font-label-sm" x-text="index + 1"></div>
                    <div class="flex-1">
                        <p class="text-sm font-bold text-gray-900 m-0 font-label-md" x-text="typeof step === 'object' ? (step.role || step.name) : step"></p>
                    </div>
                </div>
