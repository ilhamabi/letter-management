...
@section('title', 'Edit Jenis Surat: ' . $letterType->name . ' - Layanan Dokumen')
...
@section('content')
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <div class="flex items-center gap-3">
                    <h1 class="text-2xl font-bold text-gray-900 font-headline-lg m-0">Edit Jenis Surat: {{ $letterType->name }}</h1>
                    @if($letterType->is_active)
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 font-label-sm">
                            <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 mr-1.5"></span>
                            Aktif
                        </span>
                    @else
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-gray-100 text-gray-600 border border-gray-200 font-label-sm">
                            <span class="w-1.5 h-1.5 rounded-full bg-gray-400 mr-1.5"></span>
                            Tidak Aktif
                        </span>
                    @endif
                </div>
                <p class="text-sm text-gray-600 mt-1 font-body-sm">Terakhir diperbarui: {{ $letterType->updated_at ? $letterType->updated_at->translatedFormat('d F Y H:i') : '-' }}</p>
            </div>
        </div>
    </div>
    ...
    @if ($errors->any())
        <div class="bg-red-50 border border-red-200 text-red-700 px-4 py-3.5 rounded-xl flex items-start gap-3 mb-6 shadow-xs">
            ...
            <div class="space-y-1">
                <p class="font-bold text-sm font-label-md">Gagal memperbarui jenis surat. Mohon periksa inputan berikut:</p>
    ...
    <div class="max-w-4xl mx-auto space-y-8 pb-12">
        <form action="{{ route('admin.letters.update', $letterType->id) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <section class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm overflow-hidden relative">
                ...
                <div class="border-b border-gray-100 pb-4 mb-6">
                    ...
                    <p class="text-xs text-gray-500 mt-0.5 font-body-sm">Ubah nama, kode acuan, deskripsi, dan status keaktifan jenis surat.</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="flex items-center gap-4 pt-1">
                            <label class="inline-flex items-center gap-2 cursor-pointer select-none">
                                <input type="radio" name="is_active" value="1" {{ old('is_active', $letterType->is_active ? '1' : '0') == '1' ? 'checked' : '' }} class="text-amikom-purple focus:ring-amikom-purple">
                                <span class="text-sm font-semibold text-gray-800 font-body-sm">Aktif</span>
                            </label>
                            <label class="inline-flex items-center gap-2 cursor-pointer select-none">
                                <input type="radio" name="is_active" value="0" {{ old('is_active', $letterType->is_active ? '1' : '0') == '0' ? 'checked' : '' }} class="text-amikom-purple focus:ring-amikom-purple">
                    ...
                    <div class="space-y-1.5 md:col-span-2">
                        ...
                        <textarea 
                            ...
                            class="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-lg text-sm text-gray-900 focus:ring-2 focus:ring-amikom-purple focus:border-amikom-purple focus:bg-white transition-all font-body-sm">{{ old('description', $letterType->description) }}</textarea>
                    </div>
                </div>
            </section>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input 
                            ...
                            id="minimum_gpa" 
                            name="minimum_gpa" 
                            value="{{ old('minimum_gpa', $letterType->minimum_gpa) }}" 
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input 
                            ...
                            value="{{ old('minimum_credits', $letterType->minimum_credits) }}" 
                    ...
                    <div class="md:col-span-2 space-y-3 pt-2">
                        <label class="flex items-center gap-3 p-3 bg-gray-50 border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-100/80 transition-colors select-none">
                            <input type="checkbox" name="requires_attachment" value="1" {{ old('requires_attachment', $letterType->requires_attachment) ? 'checked' : '' }} class="rounded border-gray-300 text-amikom-purple focus:ring-amikom-purple w-4 h-4">
                            <div>
                        ...
                        <label class="flex items-center gap-3 p-3 bg-gray-50 border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-100/80 transition-colors select-none">
                            <input type="checkbox" name="allow_group_submission" value="1" {{ old('allow_group_submission', $letterType->allow_group_submission) ? 'checked' : '' }} class="rounded border-gray-300 text-amikom-purple focus:ring-amikom-purple w-4 h-4">
                            <div>
            ...
            <x-admin.approval-workflow-card :flows="$approvalFlows" :defaultFlow="$letterType->approvalFlow" modalId="change-workflow-modal" />

            <!-- Section 4: Konfigurasi Template Isi Surat (TinyMCE Rich Text Editor) -->
            <x-admin.letter-content-editor-card 
                editorId="letter-template-editor" 
                selectId="placeholder-select" 
                guideModalId="placeholder-guide-modal" 
                :initialContent="old('template_content', optional($letterType->activeTemplate)->body_content)" />
    ...
    <x-admin.delete-template-modal id="delete-confirmation-modal" :letterType="$letterType" />
@endsection
