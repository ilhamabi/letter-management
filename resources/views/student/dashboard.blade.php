...
@section('content')
    ...
    <section class="grid grid-cols-1 md:grid-cols-3 gap-6 w-full">
        <x-stat-card icon="pending_actions" iconBg="bg-primary-fixed" iconColor="text-primary" title="Menunggu"
            :value="$pendingCount" />
        <x-stat-card icon="check_circle" iconBg="bg-green-100" iconColor="text-green-700" title="Disetujui"
            :value="$approvedCount" />
        <x-stat-card icon="cancel" iconBg="bg-error-container" iconColor="text-error" title="Ditolak"
            :value="$rejectedCount" />
    </section>
    ...
    <section class="bg-pure-white rounded-xl border border-outline-variant shadow-sm overflow-hidden flex flex-col w-full">
        <div class="p-6 border-b border-outline-variant flex justify-between items-center">
            ...
            <a href="{{ route('student.submissions.history') }}"
                class="group inline-flex items-center gap-1 text-primary text-sm font-semibold">
                <span class="group-hover:underline">Lihat Semua</span>
            ...
            <div class="overflow-x-auto w-full">
                <table class="w-full text-left border-collapse">
                    <thead class="bg-surface-container-low/70 border-b border-outline-variant">
                        <tr>
                            <th class="text-xs font-bold uppercase tracking-wider text-on-surface-variant py-4 px-6">Jenis Surat
                            </th>
                            <th class="text-xs font-bold uppercase tracking-wider text-on-surface-variant py-4 px-6">Tanggal
                                Pengajuan</th>
                            <th
                                class="text-xs font-bold uppercase tracking-wider text-on-surface-variant py-4 px-6 text-center">
                                Status</th>
                        </tr>
                    </thead>
                    <tbody class="text-sm text-on-surface divide-y divide-outline-variant">
                        @foreach ($latestSubmissions as $sub)
                            ...
                            <tr class="hover:bg-surface-container-low/50 transition-colors duration-200 cursor-pointer"
                                ...
                                <td class="py-4 px-6 font-semibold text-deep-black">
        ...
        @else
            <div class="flex flex-col items-center justify-center py-12 px-6 text-center">
                ...
                <p class="text-sm text-on-surface-variant mb-6">Semua permintaan dokumen Anda telah selesai diproses atau belum
                    ada pengajuan baru.</p>
                <a class="group inline-flex items-center gap-2 text-primary text-sm font-semibold"
                    href="{{ route('student.submissions.history') }}">
                    <span class="group-hover:underline">Lihat Semua Riwayat</span>
